...
async function main() {
  const { data, error } = await supabase.from('chart_of_accounts').select('id, account_code, account_name, company_id(name)').eq('account_code', '13001013');
  console.log("Account 13001013:", data, error);
  
  const { data: d2 } = await supabase.from('chart_of_accounts').select('id, account_code, account_name').eq('account_code', '13001011');
  console.log("Account 13001011:", d2);
}
main();
