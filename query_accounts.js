...
async function main() {
  const { data, error } = await supabase.from('chart_of_accounts').select('id, account_code, account_name, company_id').ilike('account_name', '%advance%');
  console.log("Advances Accounts:");
  console.log(JSON.stringify(data, null, 2));

  const { data: iouJE } = await supabase.from('journal_entry_lines')
    .select('id, journal_entry_id, account_id, debit, credit, description')
    .ilike('description', '%Staff advance:%');
  console.log("IOU JE Lines:");
  console.log(JSON.stringify(iouJE, null, 2));
}
main();
