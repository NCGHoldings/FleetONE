...
export interface LightVehicleWarranty {
  id: string;
  order_id: string;
...
export interface LightVehicleServiceReminder {
  id: string;
  order_id: string;
  warranty_id?: string;
...
export interface LightVehicleSupportTicket {
  id: string;
...
export interface LightVehicleCustomerFeedback {
  id: string;
  customer_id: string;
  order_id?: string;
...
export function useLightVehicleAfterSalesManagement() {
  const [isLoading, setIsLoading] = useState(false);
  const { toast } = useToast();

  // Warranty Management
  const createWarranty = async (warrantyData: Partial<LightVehicleWarranty>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('lightvehicle_warranties')
  ...
  const getWarranties = async (orderId?: string) => {
    ...
    try {
      let query = supabase
        ...
        .select(`
          *,
          lightvehicle_orders!inner(
            order_no,
            lightvehicle_customers!inner(
              customer_name,
              contact_person,
              email,
              phone
  ...
  // Service Reminders
  const createServiceReminder = async (reminderData: Partial<LightVehicleServiceReminder>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('lightvehicle_service_reminders')
  ...
  const updateReminderStatus = async (reminderId: string, updates: Partial<LightVehicleServiceReminder>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('lightvehicle_service_reminders')
  ...
  // Support Tickets
  const createSupportTicket = async (ticketData: Partial<LightVehicleSupportTicket>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('lightvehicle_support_tickets')
  ...
  // Customer Feedback
  const createCustomerFeedback = async (feedbackData: Partial<LightVehicleCustomerFeedback>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('lightvehicle_customer_feedback')
