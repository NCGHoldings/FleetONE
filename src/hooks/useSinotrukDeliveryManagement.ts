...
export interface SinotrukDeliveryInspection {
  id: string;
  order_id: string;
...
export interface SinotrukCustomerHandover {
  id: string;
  order_id: string;
  customer_id: string;
...
export interface SinotrukDeliveryConfirmation {
  id: string;
  order_id: string;
...
export function useSinotrukDeliveryManagement() {
  const [isLoading, setIsLoading] = useState(false);
  const { toast } = useToast();

  // Delivery Inspections
  const createDeliveryInspection = async (inspectionData: Partial<SinotrukDeliveryInspection>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('sinotruck_delivery_inspections')
  ...
  // Customer Handovers
  const createCustomerHandover = async (handoverData: Partial<SinotrukCustomerHandover>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('sinotruck_customer_handovers')
  ...
  const createDeliveryConfirmation = async (confirmationData: Partial<SinotrukDeliveryConfirmation>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('sinotruck_delivery_confirmations')
