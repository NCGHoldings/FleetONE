...
export interface LightVehicleReportFilters {
...
export interface LightVehicleReportData {
...
async function fetchReportData(filters: LightVehicleReportFilters): Promise<LightVehicleReportData> {
  const now = new Date();
  ...
  const [
    ...
    supabase.from('lightvehicle_quotations').select('id, status, total_price, quantity, created_at, bus_model, customer_name, is_active_version'),
    supabase.from('lightvehicle_orders').select('id, status, total_amount, quantity, unit_price, payment_mode, current_phase, total_paid, balance_due, bus_model, customer_id, order_date, actual_delivery_date, created_at, expected_delivery_date'),
    supabase.from('lightvehicle_payment_schedules').select('id, amount, status, due_date, payment_date'),
    supabase.from('lightvehicle_letter_of_credits').select('id, status, lc_amount'),
    supabase.from('lightvehicle_shipment_groups').select('id, status, shipment_name, created_at'),
    supabase.from('lightvehicle_shipments').select('id, status'),
    supabase.from('lightvehicle_customs_declarations').select('id, customs_status'),
    supabase.from('lightvehicle_vehicle_processing').select('id, current_stage'),
    supabase.from('lightvehicle_rmv_registrations').select('id, registration_status'),
    supabase.from('lightvehicle_delivery_inspections').select('id, status'),
    supabase.from('lightvehicle_delivery_confirmations').select('id'),
    supabase.from('lightvehicle_customer_handovers').select('id, status'),
    supabase.from('lightvehicle_warranties').select('id, status'),
    supabase.from('lightvehicle_support_tickets').select('id, status, priority'),
    supabase.from('lightvehicle_service_reminders').select('id, service_completed, due_date'),
    supabase.from('lightvehicle_customer_feedback').select('id, overall_rating'),
  ]);
  ...
  const monthlyTrends: LightVehicleReportData['monthlyTrends'] = [];
  for (let i = 11; i >= 0; i--) {
  ...
  if (customerIds.length > 0) {
    const { data } = await supabase.from('lightvehicle_customers').select('id, company_name, contact_person').in('id', customerIds);
    customersData = data || [];
  }
...
export function useLightVehicleExecutiveReport(filters?: LightVehicleReportFilters) {
  const effectiveFilters: LightVehicleReportFilters = filters || {
    startDate: null,
    endDate: null,
