...
export interface YutongShipment {
  id: string;
  order_id: string;
...
export const useYutongLogisticsManagement = () => {
  ...
  const addShippingDocument = async (documentData: {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('yutong_shipping_documents')
        .insert({
