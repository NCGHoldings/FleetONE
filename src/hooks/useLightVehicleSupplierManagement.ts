...
export interface LightVehicleSupplierOrder {
  id: string;
  order_id: string;
  lightvehicle_order_reference?: string;
...
export const useLightVehicleSupplierManagement = () => {
  ...
  const createSupplierOrder = async (orderData: {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('lightvehicle_supplier_orders')
        .insert({
          ...orderData,
        })
        .select()
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          ...
          lightvehicle_production_updates (
            id,
            milestone,
            update_date,
            milestone_completed,
  ...
  const addProductionUpdate = async (updateData: {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('lightvehicle_production_updates')
        .insert({
  ...
  // Transmission to LightVehicle simulation
  const transmitOrderToLightVehicle = async (supplierOrderId: string, orderDetails: any) => {
    try {
      setIsLoading(true);

      // Simulate API call to LightVehicle
      const lightvehicleReference = `YT-${Date.now()}`;
      
      const { error } = await supabase
        .from('lightvehicle_supplier_orders')
        .update({
          lightvehicle_order_reference: lightvehicleReference,
          status: 'transmitted',
          current_milestone: 'order_received',
          production_progress_percentage: 10,
          supplier_notes: 'Order transmitted to LightVehicle successfully'
        })
        .eq('id', supplierOrderId);

      if (error) throw error;

      toast.success(`Order transmitted to LightVehicle. Reference: ${lightvehicleReference}`);
      return { success: true, lightvehicleReference };
    } catch (error) {
      console.error('Error transmitting order:', error);
      toast.error('Failed to transmit order to LightVehicle');
      return { success: false, error };
    } finally {
      setIsLoading(false);
    }
