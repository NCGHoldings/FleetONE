...
export function useSinotrukSpreadsheetData() {
    ...
    try {
      const { data: rawOrders, error: ordersError } = await supabase
        ...
        .select(`
          ...
          sinotruck_quotations(customer_name, company_name)
        `)
      ...
      const { data: deliveryOrders } = await supabase
        .from('sinotruck_delivery_orders')
      ...
      const mapped: SpreadsheetOrder[] = (rawOrders || []).map((o: any) => ({
        id: o.id,
        order_no: o.order_no || '',
        customer_name: o.sinotruck_quotations?.customer_name || '',
        company_name: o.sinotruck_quotations?.company_name || '',
  ...
  useEffect(() => {
    const channel = supabase
      .channel('sinotruck-orders-spreadsheet')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'sinotruck_orders' }, () => {
        fetchData();
      })
      .subscribe();
