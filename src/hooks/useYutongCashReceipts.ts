...
export interface YutongCashReceipt {
  id: string;
  order_id: string;
  payment_id: string;
  receipt_no: string;
...
// Convert number to words for cash receipt
...
export const useYutongCashReceipts = () => {
  ...
  const generateReceiptNumber = async (orderId: string): Promise<string> => {
    try {
      ...
      const { data: order, error: orderError } = await supabase
        .from('yutong_orders')
        .select('yutong_quotations(quotation_no)')
        .eq('id', orderId)
        .single();

      if (orderError) throw orderError;

      const quotationNo = order?.yutong_quotations?.quotation_no || 'UNKNOWN';
  ...
  const createCashReceipt = async (
    ...
    paymentDate: string
  ): Promise<YutongCashReceipt | null> => {
    try {
      setLoading(true);
      ...
      const productDescription = `ADVANCE PAYMENT FOR ${quotation?.quantity || 1} UNIT OF YUTONG - ${quotation?.bus_model || 'BUS'}`;
      ...
      return data as YutongCashReceipt;
    } catch (error: any) {
  ...
  const getCashReceiptByPaymentId = async (paymentId: string): Promise<YutongCashReceipt | null> => {
    try {
      const { data, error } = await supabase
        .from('yutong_cash_receipts')
        .select('*')
      ...
      if (error) throw error;
      return data as YutongCashReceipt | null;
    } catch (error) {
  ...
  const getCashReceiptsForOrder = async (orderId: string): Promise<YutongCashReceipt[]> => {
    try {
      const { data, error } = await supabase
        .from('yutong_cash_receipts')
        .select('*')
