...
export interface LightVehicleDeliveryInspection {
  id: string;
  order_id: string;
...
export interface LightVehicleCustomerHandover {
  id: string;
  order_id: string;
  customer_id: string;
...
export interface LightVehicleDeliveryConfirmation {
  id: string;
  order_id: string;
...
export function useLightVehicleDeliveryManagement() {
  const [isLoading, setIsLoading] = useState(false);
  const { toast } = useToast();

  // Delivery Inspections
  const createDeliveryInspection = async (inspectionData: Partial<LightVehicleDeliveryInspection>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('lightvehicle_delivery_inspections')
  ...
  // Customer Handovers
  const createCustomerHandover = async (handoverData: Partial<LightVehicleCustomerHandover>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('lightvehicle_customer_handovers')
  ...
  const createDeliveryConfirmation = async (confirmationData: Partial<LightVehicleDeliveryConfirmation>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('lightvehicle_delivery_confirmations')
