...
import { generateSinotrukInvoicePDF, type SinotrukInvoiceData } from '@/lib/sinotruck-invoice-generator';

export interface SinotrukStoredInvoice {
  id: string;
  quotation_id: string;
  invoice_no: string;
...
export interface SinotrukStoredDocument {
  id: string;
  quotation_id: string;
...
export const useSinotrukInvoiceManagement = () => {
  ...
  const generateAndStoreDraftInvoice = async (
    ...
    try {
      ...
      const pdfBlob = await generateSinotrukInvoicePDF(draftInvoiceData);
      ...
      const fileName = `DRAFT-Sinotruk-Invoice-${invoiceData.quotationNo}-${Date.now()}.pdf`;
      ...
      toast.success('Draft Sinotruk invoice generated and stored successfully');
  ...
  const approveInvoice = async (invoiceId: string, documentId: string) => {
    try {
      ...
      const quotation = (invoiceData as any).sinotruck_quotations;
      if (!quotation) throw new Error('Quotation data not found');

      // Prepare invoice data for regeneration
      const approvedInvoiceData: SinotrukInvoiceData = {
      ...
      const pdfBlob = await generateSinotrukInvoicePDF(approvedInvoiceData);
      const arrayBuffer = await pdfBlob.arrayBuffer();
      const uint8Array = new Uint8Array(arrayBuffer);
  ...
  const regenerateInvoice = async (documentId: string) => {
    try {
      ...
      const invoice = (document as any).sinotruck_invoices;
      const quotation = invoice.sinotruck_quotations;
      ...
      // Generate new PDF
      const pdfBlob = await generateSinotrukInvoicePDF(invoiceData);
      const arrayBuffer = await pdfBlob.arrayBuffer();
      const uint8Array = new Uint8Array(arrayBuffer);
