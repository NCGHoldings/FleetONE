...
export interface SinotrukShipment {
  id: string;
  order_id: string;
...
export const useSinotrukLogisticsManagement = () => {
  ...
  const getShippingPartners = async () => {
    try {
      const { data, error } = await supabase
        .from('sinotruck_shipping_partners')
        .select('*')
        .eq('is_active', true)
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          ...
          sinotruck_shipping_partners (
            partner_name,
            contact_email,
            contact_phone
          ),
          sinotruck_shipping_documents (
            id,
            document_type,
            document_status
          ),
          sinotruck_shipment_tracking (
            id,
            tracking_date,
            location,
            status
    ...
    try {
      ...
      if (trackingData) {
        const { error: trackingError } = await supabase
          .from('sinotruck_shipment_tracking')
          .insert({
  ...
  const addShippingDocument = async (documentData: {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('sinotruck_shipping_documents')
        .insert({
