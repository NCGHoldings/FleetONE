...
export interface YutongWarranty {
  id: string;
  order_id: string;
...
export interface YutongServiceReminder {
  id: string;
  order_id: string;
  warranty_id?: string;
...
export interface YutongSupportTicket {
  id: string;
...
export interface YutongCustomerFeedback {
  id: string;
  customer_id: string;
  order_id?: string;
...
export function useYutongAfterSalesManagement() {
  const [isLoading, setIsLoading] = useState(false);
  const { toast } = useToast();

  // Warranty Management
  const createWarranty = async (warrantyData: Partial<YutongWarranty>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('yutong_warranties')
  ...
  // Service Reminders
  const createServiceReminder = async (reminderData: Partial<YutongServiceReminder>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('yutong_service_reminders')
  ...
  const updateReminderStatus = async (reminderId: string, updates: Partial<YutongServiceReminder>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('yutong_service_reminders')
  ...
  // Support Tickets
  const createSupportTicket = async (ticketData: Partial<YutongSupportTicket>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('yutong_support_tickets')
  ...
  // Customer Feedback
  const createCustomerFeedback = async (feedbackData: Partial<YutongCustomerFeedback>) => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from('yutong_customer_feedback')
