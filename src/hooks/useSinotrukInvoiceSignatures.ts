...
export interface SinotrukInvoiceSignature {
  id: string;
...
export const useSinotrukInvoiceSignatures = () => {
  const [loading, setLoading] = useState(false);

  const fetchSignatures = async (invoiceRecordId: string): Promise<SinotrukInvoiceSignature[]> => {
    try {
      setLoading(true);
      ...
      return (data || []) as SinotrukInvoiceSignature[];
    } catch (error: any) {
