...
export interface YutongSignature {
  id: string;
  quotation_id: string;
...
export const useYutongSignatures = () => {
  const [loading, setLoading] = useState(false);

  const fetchSignatures = async (quotationId: string): Promise<YutongSignature[]> => {
    try {
      setLoading(true);
      ...
      return (data || []) as YutongSignature[];
    } catch (error) {
