...
export interface SinotrukReportFilters {
...
export interface SinotrukReportData {
...
async function fetchReportData(filters: SinotrukReportFilters): Promise<SinotrukReportData> {
  const now = new Date();
  ...
  const [
    ...
    supabase.from('sinotruck_quotations').select('id, status, total_price, quantity, created_at, bus_model, customer_name, is_active_version'),
    supabase.from('sinotruck_orders').select('id, status, total_amount, quantity, unit_price, payment_mode, current_phase, total_paid, balance_due, bus_model, customer_id, order_date, actual_delivery_date, created_at, expected_delivery_date'),
    supabase.from('sinotruck_payment_schedules').select('id, amount, status, due_date, payment_date'),
    supabase.from('sinotruck_letter_of_credits').select('id, status, lc_amount'),
    supabase.from('sinotruck_shipment_groups').select('id, status, shipment_name, created_at'),
    supabase.from('sinotruck_shipments').select('id, status'),
    supabase.from('sinotruck_customs_declarations').select('id, customs_status'),
    supabase.from('sinotruck_vehicle_processing').select('id, current_stage'),
    supabase.from('sinotruck_rmv_registrations').select('id, registration_status'),
    supabase.from('sinotruck_delivery_inspections').select('id, status'),
    supabase.from('sinotruck_delivery_confirmations').select('id'),
    supabase.from('sinotruck_customer_handovers').select('id, status'),
    supabase.from('sinotruck_warranties').select('id, status'),
    supabase.from('sinotruck_support_tickets').select('id, status, priority'),
    supabase.from('sinotruck_service_reminders').select('id, service_completed, due_date'),
    supabase.from('sinotruck_customer_feedback').select('id, overall_rating'),
  ]);
  ...
  const monthlyTrends: SinotrukReportData['monthlyTrends'] = [];
  for (let i = 11; i >= 0; i--) {
  ...
  if (customerIds.length > 0) {
    const { data } = await supabase.from('sinotruck_customers').select('id, company_name, contact_person').in('id', customerIds);
    customersData = data || [];
  }
...
export function useSinotrukExecutiveReport(filters?: SinotrukReportFilters) {
  const effectiveFilters: SinotrukReportFilters = filters || {
    startDate: null,
    endDate: null,
  ...
  return useQuery({
    queryKey: ['sinotruck-executive-report', effectiveFilters],
