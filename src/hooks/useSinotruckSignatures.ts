...
export interface SinotruckSignature {
  id: string;
  quotation_id: string;
  signature_role: 'sales_manager' | 'finance_department' | 'customer';
  signer_name: string;
...
export const useSinotruckSignatures = () => {
  const [loading, setLoading] = useState(false);

  const fetchSignatures = async (quotationId: string): Promise<SinotruckSignature[]> => {
    try {
      setLoading(true);
      ...
      return (data || []) as SinotruckSignature[];
    } catch (error) {
  ...
  const saveSignature = async (
    quotationId: string,
    signatureRole: 'sales_manager' | 'finance_department' | 'customer',
    signerName: string,
