...
export interface SinotrukSupplierOrder {
  id: string;
  order_id: string;
  sinotruck_order_reference?: string;
...
export const useSinotrukSupplierManagement = () => {
  ...
  const createSupplierOrder = async (orderData: {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('sinotruck_supplier_orders')
        .insert({
          ...orderData,
        })
        .select()
  ...
  const addProductionUpdate = async (updateData: {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('sinotruck_production_updates')
        .insert({
  ...
  // Transmission to Sinotruk simulation
  const transmitOrderToSinotruk = async (supplierOrderId: string, orderDetails: any) => {
    try {
      setIsLoading(true);

      // Simulate API call to Sinotruk
      const sinotruckReference = `YT-${Date.now()}`;
      
      const { error } = await supabase
        .from('sinotruck_supplier_orders')
        .update({
          sinotruck_order_reference: sinotruckReference,
          status: 'transmitted',
          current_milestone: 'order_received',
          production_progress_percentage: 10,
          supplier_notes: 'Order transmitted to Sinotruk successfully'
        })
        .eq('id', supplierOrderId);

      if (error) throw error;

      toast.success(`Order transmitted to Sinotruk. Reference: ${sinotruckReference}`);
      return { success: true, sinotruckReference };
    } catch (error) {
      console.error('Error transmitting order:', error);
      toast.error('Failed to transmit order to Sinotruk');
      return { success: false, error };
    } finally {
      setIsLoading(false);
    }
