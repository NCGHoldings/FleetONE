...
export function useSinotrukVehicleDataManagement() {
  ...
  const createDataSheet = useCallback(async (
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('sinotruck_vehicle_data_sheets')
        .insert({
  ...
  const insertVehicleRecords = useCallback(async (
    ...
    try {
      ...
      const { error } = await supabase
        .from('sinotruck_vehicle_records')
        .insert(insertData);
  ...
  const fetchDataSheets = useCallback(async (): Promise<VehicleDataSheet[]> => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          shipment_group:sinotruck_shipment_groups(shipment_no, shipment_name)
        `)
  ...
  const getStats = useCallback(async () => {
    try {
      const [sheetsRes, vehiclesRes] = await Promise.all([
        supabase.from('sinotruck_vehicle_data_sheets').select('id, status'),
        supabase.from('sinotruck_vehicle_records').select('id, is_matched, match_status')
      ]);
