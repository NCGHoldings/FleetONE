...
interface YutongAddOrdersToShipmentModalProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export function YutongAddOrdersToShipmentModal({
  open,
  onClose,
  onSuccess,
  shipment
}: YutongAddOrdersToShipmentModalProps) {
  const [unassignedOrders, setUnassignedOrders] = useState<any[]>([]);
  const [selectedOrderIds, setSelectedOrderIds] = useState<string[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [loading, setLoading] = useState(false);

  const { getUnassignedOrders, addOrdersToGroup, isLoading } = useYutongShipmentGroupManagement();
