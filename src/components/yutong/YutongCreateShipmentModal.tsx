...
import { useYutongLogisticsManagement, ShippingPartner } from '@/hooks/useYutongLogisticsManagement';
import { useYutongOrderManagement, YutongOrder } from '@/hooks/useYutongOrderManagement';
import { Ship } from 'lucide-react';

interface YutongCreateShipmentModalProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export function YutongCreateShipmentModal({
  open,
  onClose,
  onSuccess
}: YutongCreateShipmentModalProps) {
  ...
  const { getOrdersWithDetails } = useYutongOrderManagement();
  ...
  const loadData = async () => {
    try {
      ...
      if (ordersResult.success) {
        setOrders((ordersResult.orders || []) as YutongOrder[]);
      }
    } catch (error) {
