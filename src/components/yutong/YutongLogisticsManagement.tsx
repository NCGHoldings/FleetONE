...
import { useYutongLogisticsManagement, YutongShipment } from '@/hooks/useYutongLogisticsManagement';
import { YutongShipmentDetailsModal } from './YutongShipmentDetailsModal';
import { YutongCreateShipmentModal } from './YutongCreateShipmentModal';
...
export function YutongLogisticsManagement() {
  const [shipments, setShipments] = useState<YutongShipment[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedShipment, setSelectedShipment] = useState<YutongShipment | null>(null);
  const [showShipmentDetails, setShowShipmentDetails] = useState(false);
  const [showCreateShipment, setShowCreateShipment] = useState(false);
  
  const { getShipments, updateShipmentStatus } = useYutongLogisticsManagement();
  ...
  const columns: ColumnDef<YutongShipment>[] = [
    {
      header: "Shipment Info",
      cell: ({ row }) => {
        const shipment = row.original;
        const order = (shipment as any).yutong_orders;
        return (
          <div>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const partner = (row.original as any).yutong_shipping_partners;
        return partner ? (
          <div>
