...
interface YutongReferralHistoryModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  agent: ReferralAgent;
}

export function YutongReferralHistoryModal({
  open,
  onOpenChange,
  agent
}: YutongReferralHistoryModalProps) {
  ...
  const loadHistory = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          yutong_quotation:yutong_quotations(
            quotation_no,
            customer_name,
            total_price
          )
              ...
              {records.map((record) => (
                <TableRow key={record.id}>
                  ...
                  <TableCell className="font-mono text-sm">
                    {record.yutong_quotation?.quotation_no || '-'}
                  </TableCell>
                  <TableCell>
                    {record.yutong_quotation?.customer_name || '-'}
                  </TableCell>
                  <TableCell className="text-right">
                    {record.yutong_quotation?.total_price 
                      ? formatCurrency(Number(record.yutong_quotation.total_price))
                      : '-'}
                  </TableCell>
