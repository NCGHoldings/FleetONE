...
export function SpreadsheetRowActions({ orderId, orderNo, quotationId, hasInvoices, onViewOrder, onDeleteOrder }: SpreadsheetRowActionsProps) {
    const handleOpenInYutong = () => {
        // Navigate to full Yutong order details page
        window.location.href = `/yutong?tab=orders&orderId=${orderId}`;
    };

    const handleViewQuotation = () => {
        if (quotationId) {
            window.location.href = `/yutong?tab=quotations&quotationId=${quotationId}`;
        }
    };

    const handleGenerateInvoice = () => {
        // Open order in Yutong with invoice tab active
        window.location.href = `/yutong?tab=orders&orderId=${orderId}&action=invoice`;
    };

    const handleViewPayments = () => {
        window.location.href = `/yutong?tab=finance&orderId=${orderId}`;
    };
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
                <DropdownMenuItem onClick={handleOpenInYutong} className="text-xs gap-2">
