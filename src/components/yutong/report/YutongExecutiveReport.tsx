...
import { YutongShipmentStatus } from './YutongShipmentStatus';
import { YutongModelPerformance } from './YutongModelPerformance';
  ...
  return (
    <div className="space-y-5 max-w-[1400px] mx-auto print:max-w-none">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 print:flex-row">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Yutong Bus Sales — Executive Report</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-1">
          <YutongPipelineFunnel data={data} />
        </div>
        <div className="md:col-span-2">
          <YutongRevenueCharts data={data} />
        </div>
      </div>
      ...
      {/* After Sales */}
      <YutongAfterSalesHealth data={data} />
      ...
      <div className="hidden print:block text-center text-xs text-muted-foreground border-t pt-4 mt-8">
        NCG Holdings — Yutong Bus Sales Executive Report — Generated {new Date(data.generatedAt).toLocaleString()} — Confidential
      </div>
    </div>
  );
}
