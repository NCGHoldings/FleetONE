...
import { useYutongSupplierManagement, YutongSupplierOrder } from '@/hooks/useYutongSupplierManagement';
import { YutongSupplierOrderDetailsModal } from './YutongSupplierOrderDetailsModal';
...
export function YutongSupplierManagement() {
  const [supplierOrders, setSupplierOrders] = useState<YutongSupplierOrder[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedOrder, setSelectedOrder] = useState<YutongSupplierOrder | null>(null);
  ...
  const handleTransmitOrder = async (supplierOrderId: string) => {
    const result = await transmitOrderToYutong(supplierOrderId, {});
    if (result.success) {
      loadSupplierOrders();
    }
  };
  ...
  const columns: ColumnDef<YutongSupplierOrder>[] = [
    {
      header: "Order Details",
      cell: ({ row }) => {
        const order = (row.original as any).yutong_orders;
        return (
          <div>
    ...
    {
      accessorKey: "yutong_order_reference",
      header: "Yutong Ref",
      cell: ({ row }) => {
        const ref = row.getValue("yutong_order_reference") as string;
        return ref ? (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const hasYutongRef = Boolean(supplierOrder.yutong_order_reference);
      ...
      <Card>
        ...
        <CardContent>
          <DataTable 
            columns={columns} 
            data={supplierOrders}
            searchKey="yutong_order_reference"
          />
        </CardContent>
      </Card>

      {showOrderDetails && selectedOrder && (
        <YutongSupplierOrderDetailsModal
