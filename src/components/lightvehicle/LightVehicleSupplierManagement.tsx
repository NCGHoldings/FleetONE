...
import { useLightVehicleSupplierManagement, LightVehicleSupplierOrder } from '@/hooks/useLightVehicleSupplierManagement';
import { LightVehicleSupplierOrderDetailsModal } from './LightVehicleSupplierOrderDetailsModal';
...
export function LightVehicleSupplierManagement() {
  const [supplierOrders, setSupplierOrders] = useState<LightVehicleSupplierOrder[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedOrder, setSelectedOrder] = useState<LightVehicleSupplierOrder | null>(null);
  const [showOrderDetails, setShowOrderDetails] = useState(false);
  
  const { 
    getSupplierOrders, 
    updateSupplierOrderMilestone, 
    transmitOrderToLightVehicle,
    milestoneLabels 
  } = useLightVehicleSupplierManagement();
  ...
  const handleTransmitOrder = async (supplierOrderId: string) => {
    const result = await transmitOrderToLightVehicle(supplierOrderId, {});
    if (result.success) {
      loadSupplierOrders();
    }
  };
  ...
  const columns: ColumnDef<LightVehicleSupplierOrder>[] = [
    {
      header: "Order Details",
      cell: ({ row }) => {
        const order = (row.original as any).lightvehicle_orders;
        return (
          <div>
    ...
    {
      accessorKey: "lightvehicle_order_reference",
      header: "LightVehicle Ref",
      cell: ({ row }) => {
        const ref = row.getValue("lightvehicle_order_reference") as string;
        return ref ? (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const hasLightVehicleRef = Boolean(supplierOrder.lightvehicle_order_reference);
          ...
          <div className="flex space-x-1">
            ...
            {!hasLightVehicleRef && (
              <Button 
                ...
                title="Transmit to LightVehicle"
              >
      ...
      <Card>
        ...
        <CardContent>
          <DataTable 
            columns={columns} 
            data={supplierOrders}
            searchKey="lightvehicle_order_reference"
          />
        </CardContent>
      </Card>

      {showOrderDetails && selectedOrder && (
        <LightVehicleSupplierOrderDetailsModal
