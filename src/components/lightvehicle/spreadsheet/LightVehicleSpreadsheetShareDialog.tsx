...
export function LightVehicleSpreadsheetShareDialog({ open, onOpenChange }: Props) {
  ...
  useEffect(() => {
    if (open) {
      supabase
        ...
        .eq('setting_key', 'lightvehicle_spreadsheet_access_code')
        .single()
        .then(({ data }) => {
          if (data) {
  ...
  const saveCode = async () => {
    const { error } = await supabase.from('system_settings').upsert({
      setting_key: 'lightvehicle_spreadsheet_access_code',
  ...
  const shareUrl = `${window.location.origin}/public/lightvehicle-spreadsheet?code=${encodeURIComponent(accessCode)}`;
