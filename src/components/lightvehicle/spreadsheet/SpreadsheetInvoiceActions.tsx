...
export function SpreadsheetRowActions({ orderId, orderNo, quotationId, hasInvoices, onViewOrder, onDeleteOrder }: SpreadsheetRowActionsProps) {
    const handleOpenInLightVehicle = () => {
        // Navigate to full LightVehicle order details page
        window.location.href = `/lightvehicle?tab=orders&orderId=${orderId}`;
    };

    const handleViewQuotation = () => {
        if (quotationId) {
            window.location.href = `/lightvehicle?tab=quotations&quotationId=${quotationId}`;
        }
    };

    const handleGenerateInvoice = () => {
        // Open order in LightVehicle with invoice tab active
        window.location.href = `/lightvehicle?tab=orders&orderId=${orderId}&action=invoice`;
    };

    const handleViewPayments = () => {
        window.location.href = `/lightvehicle?tab=finance&orderId=${orderId}`;
    };
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
                <DropdownMenuItem onClick={handleOpenInLightVehicle} className="text-xs gap-2">
