...
import { type LightVehicleInvoiceData } from '@/lib/lightvehicle-invoice-generator';
import { LightVehicleInvoiceViewModal } from './LightVehicleInvoiceViewModal';
...
interface LightVehicleInvoiceGeneratorProps {
  isOpen: boolean;
  onClose: () => void;
...
export const LightVehicleInvoiceGenerator: React.FC<LightVehicleInvoiceGeneratorProps> = ({
  isOpen,
  onClose,
  quotation,
  onSuccess
  ...
  const { 
    ...
    getInvoiceDocuments, 
    isLoading 
  } = useLightVehicleInvoiceManagement();
  
  const [existingDocuments, setExistingDocuments] = useState<LightVehicleStoredDocument[]>([]);
  const [selectedDocument, setSelectedDocument] = useState<LightVehicleStoredDocument | null>(null);
  ...
  const handleViewDocument = (document: LightVehicleStoredDocument) => {
