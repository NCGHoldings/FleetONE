...
import { useLightVehicleSignatures } from '@/hooks/useLightVehicleSignatures';
import { Pencil, Type, Upload, UserCheck } from 'lucide-react';

interface LightVehicleQuotationSignatureModalProps {
  isOpen: boolean;
  onClose: () => void;
  quotationId: string;
...
export const LightVehicleQuotationSignatureModal = ({
  isOpen,
  onClose,
  quotationId,
  defaultRole,
  defaultSignerName,
  onSignatureSaved
}: LightVehicleQuotationSignatureModalProps) => {
  ...
  const { loading, saveSignature, getProfileSignature } = useLightVehicleSignatures();
