...
import { useLightVehicleExecutiveReport, LightVehicleReportFilters as FilterType } from '@/hooks/useLightVehicleExecutiveReport';
import { LightVehicleReportKPICards } from './LightVehicleReportKPICards';
import { LightVehiclePipelineFunnel } from './LightVehiclePipelineFunnel';
import { LightVehicleRevenueCharts } from './LightVehicleRevenueCharts';
import { LightVehicleShipmentStatus } from './LightVehicleShipmentStatus';
import { LightVehicleModelPerformance } from './LightVehicleModelPerformance';
import { LightVehicleTopCustomers } from './LightVehicleTopCustomers';
import { LightVehicleAfterSalesHealth } from './LightVehicleAfterSalesHealth';
import { LightVehicleReportShareDialog } from './LightVehicleReportShareDialog';
import { LightVehicleReportFilters } from './LightVehicleReportFilters';
import { LightVehicleExecutiveSummary } from './LightVehicleExecutiveSummary';
import { LightVehiclePeriodComparison } from './LightVehiclePeriodComparison';
import { LightVehicleRevenueForecasting } from './LightVehicleRevenueForecasting';
import { LightVehicleSalesVelocity } from './LightVehicleSalesVelocity';
import { LightVehicleCustomerAnalytics } from './LightVehicleCustomerAnalytics';
import { exportLightVehicleReport } from './LightVehicleExcelExporter';
...
function PredictiveInsights({ data }: { data: LightVehicleReportData }) {
...
export function LightVehicleExecutiveReport({ data: externalData, isPublic = false }: Props) {
  ...
  const { data: hookData, isLoading, refetch } = useLightVehicleExecutiveReport(externalData ? undefined : filters);
  ...
  return (
    <div className="space-y-5 max-w-[1400px] mx-auto print:max-w-none">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 print:flex-row">
        <div>
          <h1 className="text-2xl font-bold text-foreground">LightVehicle Bus Sales — Executive Report</h1>
        ...
        {!isPublic && (
          <div className="flex gap-2 print:hidden">
            ...
            <Button variant="outline" size="sm" onClick={() => exportLightVehicleReport(data)} className="gap-2">
              <Download className="h-4 w-4" /> Export Excel
            </Button>
            <LightVehicleReportShareDialog />
          </div>
        )}
      </div>
      ...
      {/* Executive Summary */}
      <LightVehicleExecutiveSummary data={data} />

      {/* KPI Cards */}
      <LightVehicleReportKPICards data={data} />

      {/* Period Comparison */}
      <LightVehiclePeriodComparison data={data} />

      {/* Pipeline + Revenue */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-1">
          <LightVehiclePipelineFunnel data={data} />
        </div>
        <div className="md:col-span-2">
          <LightVehicleRevenueCharts data={data} />
        </div>
      </div>

      {/* Sales Velocity & Conversion */}
      <LightVehicleSalesVelocity data={data} />

      {/* Revenue Forecasting & Breakdown */}
      <LightVehicleRevenueForecasting data={data} />

      {/* Shipment & Operations */}
      <LightVehicleShipmentStatus data={data} />
      ...
      {/* Bus Models */}
      <LightVehicleModelPerformance data={data} />

      {/* Customer Analytics */}
      <LightVehicleCustomerAnalytics data={data} />

      {/* Top Customers (full table) */}
      <LightVehicleTopCustomers data={data} />

      {/* After Sales */}
      <LightVehicleAfterSalesHealth data={data} />
      ...
      <div className="hidden print:block text-center text-xs text-muted-foreground border-t pt-4 mt-8">
        NCG Holdings — LightVehicle Bus Sales Executive Report — Generated {new Date(data.generatedAt).toLocaleString()} — Confidential
      </div>
    </div>
  );
}
