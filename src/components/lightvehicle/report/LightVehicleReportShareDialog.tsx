...
export function LightVehicleReportShareDialog() {
  ...
  const loadCode = async () => {
    const { data } = await supabase
      ...
      .eq('setting_key', 'lightvehicle_report_access_code')
      .maybeSingle();
  ...
  const generateCode = async () => {
    ...
    const { error } = await supabase
      .from('system_settings')
      .upsert({ setting_key: 'lightvehicle_report_access_code', setting_value: newCode as any }, { onConflict: 'setting_key' });
  ...
  const shareUrl = `${window.location.origin}/public/lightvehicle-report?code=${code}`;
