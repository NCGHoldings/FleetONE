...
interface LightVehicleAddOrdersToShipmentModalProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export function LightVehicleAddOrdersToShipmentModal({
  open,
  onClose,
  onSuccess,
  shipment
}: LightVehicleAddOrdersToShipmentModalProps) {
  const [unassignedOrders, setUnassignedOrders] = useState<any[]>([]);
  const [selectedOrderIds, setSelectedOrderIds] = useState<string[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [loading, setLoading] = useState(false);

  const { getUnassignedOrders, addOrdersToGroup, isLoading } = useLightVehicleShipmentGroupManagement();
