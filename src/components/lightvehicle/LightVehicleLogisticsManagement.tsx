...
import { useLightVehicleLogisticsManagement, LightVehicleShipment } from '@/hooks/useLightVehicleLogisticsManagement';
import { LightVehicleShipmentDetailsModal } from './LightVehicleShipmentDetailsModal';
import { LightVehicleCreateShipmentModal } from './LightVehicleCreateShipmentModal';
...
export function LightVehicleLogisticsManagement() {
  const [shipments, setShipments] = useState<LightVehicleShipment[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedShipment, setSelectedShipment] = useState<LightVehicleShipment | null>(null);
  const [showShipmentDetails, setShowShipmentDetails] = useState(false);
  const [showCreateShipment, setShowCreateShipment] = useState(false);
  
  const { getShipments, updateShipmentStatus } = useLightVehicleLogisticsManagement();
  ...
  const columns: ColumnDef<LightVehicleShipment>[] = [
    {
      header: "Shipment Info",
      cell: ({ row }) => {
        const shipment = row.original;
        const order = (shipment as any).lightvehicle_orders;
        return (
          <div>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const partner = (row.original as any).lightvehicle_shipping_partners;
        return partner ? (
          <div>
  ...
  return (
    <>
      ...
      {showShipmentDetails && selectedShipment && (
        <LightVehicleShipmentDetailsModal
      ...
      {showCreateShipment && (
        <LightVehicleCreateShipmentModal
