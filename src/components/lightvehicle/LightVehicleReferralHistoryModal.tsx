...
interface LightVehicleReferralHistoryModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  agent: ReferralAgent;
}

export function LightVehicleReferralHistoryModal({
  open,
  onOpenChange,
  agent
}: LightVehicleReferralHistoryModalProps) {
  ...
  const loadHistory = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          lightvehicle_quotation:lightvehicle_quotations(
            quotation_no,
            customer_name,
            total_price
          )
              ...
              {records.map((record) => (
                <TableRow key={record.id}>
                  ...
                  <TableCell className="font-mono text-sm">
                    {record.lightvehicle_quotation?.quotation_no || '-'}
                  </TableCell>
                  <TableCell>
                    {record.lightvehicle_quotation?.customer_name || '-'}
                  </TableCell>
                  <TableCell className="text-right">
                    {record.lightvehicle_quotation?.total_price 
                      ? formatCurrency(Number(record.lightvehicle_quotation.total_price))
                      : '-'}
                  </TableCell>
