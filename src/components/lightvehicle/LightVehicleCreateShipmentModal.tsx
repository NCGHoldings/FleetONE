...
import { useLightVehicleLogisticsManagement, ShippingPartner } from '@/hooks/useLightVehicleLogisticsManagement';
import { useLightVehicleOrderManagement, LightVehicleOrder } from '@/hooks/useLightVehicleOrderManagement';
import { Ship } from 'lucide-react';

interface LightVehicleCreateShipmentModalProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export function LightVehicleCreateShipmentModal({
  open,
  onClose,
  onSuccess
}: LightVehicleCreateShipmentModalProps) {
  ...
  const { 
    getShippingPartners, 
    createShipment 
  } = useLightVehicleLogisticsManagement();
  
  const { getOrdersWithDetails } = useLightVehicleOrderManagement();
  ...
  const loadData = async () => {
    try {
      ...
      if (ordersResult.success) {
        setOrders((ordersResult.orders || []) as LightVehicleOrder[]);
      }
    } catch (error) {
