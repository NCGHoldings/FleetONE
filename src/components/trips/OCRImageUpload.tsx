...
export function OCRImageUpload({ selectedDate, onDataExtracted }: OCRImageUploadProps) {
  ...
  const applyMultiTripData = async (data: ExtractedMultiTripData) => {
    try {
      ...
      // 5. MAP OCR EXPENSES TO DB SCHEMA, then automatically save daily expenses
      ...
      const actionMsg = updatedTripNos.length > 0 
        ? `Updated ${updatedTripNos.length}, Added ${insertedTripNos.length}`
        : `Added ${insertedTripNos.length}`;
      
      toast.success(
        `✅ Trips: ${actionMsg} (${allTripNos.join(', ')}) | Revenue: Rs. ${totalRevenue.toLocaleString()}`,
        {
          duration: 6000,
          action: {
            label: 'View Expenses',
            onClick: () => {
              window.location.href = `/daily-bus-expenses?date=${tripDate}&bus=${busData.id}`;
            }
          }
        }
      );
