...
import { generateSinotruckOrderInvoiceHTML, generateSinotruckOrderInvoicePDF } from '@/lib/sinotruck-order-invoice-generator';
import { SinotruckInvoiceSignatureManager } from './SinotruckInvoiceSignatureManager';
import { SinotruckOrderInvoicePreview } from './SinotruckOrderInvoicePreview';

interface SinotruckOrderInvoiceViewModalProps {
  isOpen: boolean;
  onClose: () => void;
  document: any;
  onRefresh?: () => void;
...
export function SinotruckOrderInvoiceViewModal({
  isOpen,
  onClose,
  document,
  onRefresh
}: SinotruckOrderInvoiceViewModalProps) {
  const [isSendingEmail, setIsSendingEmail] = useState(false);
  const [refreshKey, setRefreshKey] = useState(0);
  const { isLoading, approveInvoice, regenerateInvoice } = useSinotruckOrderInvoiceManagement();

  const invoiceHTML = generateSinotruckOrderInvoiceHTML(document.invoice_data);
  ...
  const handleDownload = async () => {
    try {
      ...
      const pdfBlob = await generateSinotruckOrderInvoicePDF(mergedData);
  ...
  const handlePrint = async () => {
    try {
      ...
      const invoiceHTML = generateSinotruckOrderInvoiceHTML(mergedData);
      ...
      <DialogContent className="max-w-6xl max-h-[90vh]">
        ...
        <Tabs defaultValue="preview" className="w-full">
          ...
          <TabsContent value="signatures" className="space-y-4 max-h-[calc(90vh-280px)] overflow-auto">
            <SinotruckInvoiceSignatureManager
              invoiceRecordId={document.invoice_record_id}
              onSignaturesUpdated={() => {
