...
import { SinotrukQuotationPreview } from './SinotrukQuotationPreview';
import { SinotrukSignatureManager } from './SinotrukSignatureManager';
...
interface SinotrukQuotationViewModalProps {
  quotation: SinotrukQuotation | null;
  open: boolean;
  onClose: () => void;
}

export function SinotrukQuotationViewModal({ quotation, open, onClose }: SinotrukQuotationViewModalProps) {
  ...
  const handlePrint = () => {
    if (printRef.current) {
      ...
      if (printWindow) {
        printWindow.document.write('<html><head><title>Sinotruk Quotation</title></head><body>');
  ...
  const handleDownload = async () => {
    try {
      ...
      link.href = url;
      link.download = `Sinotruk-Quotation-${quotation.quotation_no}.pdf`;
    ...
    try {
      ...
      const response = await fetch('/functions/v1/send-quotation-email', {
        ...
        body: JSON.stringify({
          ...
          subject: `Sinotruk Bus Quotation - ${quotation.quotation_no}`,
          html: `
            <p>Dear ${quotation.customer_name},</p>
            <p>Please find attached your Sinotruk bus quotation for ${quotation.bus_model}.</p>
            <p>If you have any questions, please don't hesitate to contact us.</p>
            <p>Best regards,<br/>NCG Express Sales Team</p>
          `,
          attachment: {
            filename: `Sinotruk-Quotation-${quotation.quotation_no}.pdf`,
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full mt-4">
          ...
          <TabsContent value="preview" className="mt-6">
            <div ref={printRef}>
              <SinotrukQuotationPreview quotation={quotation} key={refreshKey} />
            </div>
          </TabsContent>

          <TabsContent value="signatures" className="mt-6">
            <SinotrukSignatureManager 
