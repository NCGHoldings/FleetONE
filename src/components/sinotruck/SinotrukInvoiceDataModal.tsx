...
interface SinotrukInvoiceDataModalProps {
  isOpen: boolean;
  onClose: () => void;
  orderId: string;
  existingData?: {
...
export function SinotrukInvoiceDataModal({
  isOpen,
  onClose,
  orderId,
  existingData,
  onSuccess
}: SinotrukInvoiceDataModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Invalidate all sinotruck orders queries to trigger UI refresh
      await queryClient.invalidateQueries({ queryKey: ['sinotruck-orders'] });
      await queryClient.invalidateQueries({ queryKey: ['sinotruck-order', orderId] });
