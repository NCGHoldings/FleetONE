...
interface SinotruckInvoiceTypeModalProps {
  ...
  onClose: () => void;
  totalAmount: number;
  onConfirm: (config: SinotruckProformaInvoiceConfig) => void;
  isLoading?: boolean;
  ...
  const handleConfirm = () => {
    const config: SinotruckProformaInvoiceConfig = {
      ...
      ...(invoiceCategory === 'tax_invoice' && {
        isTaxInvoice: true,
        customerVatNumber: purchaserTin || undefined,
        taxRate,
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          >
            <div className="flex items-start space-x-3 p-4 border rounded-lg hover:border-primary transition-colors cursor-pointer">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground mt-1">Standard invoice for full amount directly to customer</p>
                <p className="text-sm font-medium mt-2">Amount: LKR {totalAmount.toLocaleString()}</p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3 p-4 border rounded-lg hover:border-primary transition-colors cursor-pointer">
              ...
              <div className="flex-1">
                <Label htmlFor="tax" className="font-semibold text-base cursor-pointer">Tax Invoice (Sri Lanka Government Format)</Label>
                <p className="text-sm text-muted-foreground mt-1">Government-mandated format with VAT breakdown</p>
                <p className="text-sm font-medium mt-2">Amount: LKR {totalAmount.toLocaleString()} (incl. {taxRate}% VAT)</p>
              </div>
            </div>
          </RadioGroup>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={isLoading}>{isLoading ? 'Generating...' : 'Generate Invoice'}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
