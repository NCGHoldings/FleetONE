...
import { SinotrukCustomerCard } from "./SinotrukCustomerCard";
import { SinotrukCustomerLinkModal } from "./SinotrukCustomerLinkModal";
import { SinotrukCustomerDetailModal } from "./SinotrukCustomerDetailModal";
import { useSinotrukQuotationCards } from "@/hooks/useSinotrukQuotationCards";
import { useToast } from "@/hooks/use-toast";
import { Skeleton } from "@/components/ui/skeleton";

interface SinotrukCustomerCardViewProps {
  canManageLinks: boolean;
}

export function SinotrukCustomerCardView({ canManageLinks }: SinotrukCustomerCardViewProps) {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedCustomerName, setSelectedCustomerName] = useState<string | null>(null);
  const [linkModalOpen, setLinkModalOpen] = useState(false);
  const [detailModalOpen, setDetailModalOpen] = useState(false);
  const { cards, loading, loadCards, linkSubCustomer } = useSinotrukQuotationCards();
