...
import { useSinotrukSignatures } from '@/hooks/useSinotrukSignatures';
import { Pencil, Type, Upload, UserCheck } from 'lucide-react';

interface SinotrukQuotationSignatureModalProps {
  isOpen: boolean;
  onClose: () => void;
  quotationId: string;
...
export const SinotrukQuotationSignatureModal = ({
  isOpen,
  onClose,
  quotationId,
  defaultRole,
  defaultSignerName,
  onSignatureSaved
}: SinotrukQuotationSignatureModalProps) => {
  ...
  const { loading, saveSignature, getProfileSignature } = useSinotrukSignatures();
