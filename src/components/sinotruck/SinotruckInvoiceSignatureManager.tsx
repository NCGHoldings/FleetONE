...
import { SinotruckInvoiceSignatureModal } from './SinotruckInvoiceSignatureModal';
import { useSinotruckInvoiceSignatures, SinotruckInvoiceSignature } from '@/hooks/useSinotruckInvoiceSignatures';
import { Plus, Trash2, PenTool, Type, Image as ImageIcon } from 'lucide-react';
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle } from '@/components/ui/alert-dialog';

interface SinotruckInvoiceSignatureManagerProps {
  invoiceRecordId: string;
  onSignaturesUpdated?: () => void;
}

export function SinotruckInvoiceSignatureManager({
  invoiceRecordId,
  onSignaturesUpdated
}: SinotruckInvoiceSignatureManagerProps) {
  const [signatures, setSignatures] = useState<SinotruckInvoiceSignature[]>([]);
  ...
  const { fetchSignatures, deleteSignature, loading } = useSinotruckInvoiceSignatures();
  ...
  const renderSignaturePreview = (signature: SinotruckInvoiceSignature) => {
  ...
  return (
    <>
      ...
      <SinotruckInvoiceSignatureModal
        isOpen={isModalOpen}
