...
interface SinotrukEditQuotationModalProps {
...
export function SinotrukEditQuotationModal({ quotation, open, onClose, onSuccess }: SinotrukEditQuotationModalProps) {
  ...
  const loadQuotationAddOns = async () => {
    ...
    try {
      ...
      const transformedAddOns = (data || []).map((item: any) => ({
        ...
        addon_name: item.sinotruck_addons?.addon_name || '',
        category: item.sinotruck_addons?.category || '',
  ...
  const handleFormSubmit = async (data: FormData) => {
    try {
      ...
      // Generate next version number using Sinotruk-specific function
      const { data: versionData, error: versionError } = await supabase
        .rpc('generate_next_sinotruck_version_number' as any, { p_parent_id: quotation.id });
