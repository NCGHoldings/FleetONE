...
import { SinotrukQuotationViewModal } from './SinotrukQuotationViewModal';
import { SinotrukInvoiceGenerator } from './SinotrukInvoiceGenerator';
import { SinotrukEditQuotationModal } from './SinotrukEditQuotationModal';
import { SinotrukCustomerCardView } from './SinotrukCustomerCardView';
import { SinotrukQuotationRepeatModal } from './SinotrukQuotationRepeatModal';
...
interface SinotrukQuotationsListProps {
  onRefresh: () => void;
}

export function SinotrukQuotationsList({ onRefresh }: SinotrukQuotationsListProps) {
  const [quotations, setQuotations] = useState<SinotrukQuotation[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedQuotation, setSelectedQuotation] = useState<SinotrukQuotation | null>(null);
  const [viewModalOpen, setViewModalOpen] = useState(false);
  const [invoiceGeneratorOpen, setInvoiceGeneratorOpen] = useState(false);
  const [editModalOpen, setEditModalOpen] = useState(false);
  const [repeatModalOpen, setRepeatModalOpen] = useState(false);
  const [viewMode, setViewMode] = useState<'table' | 'card'>(() => {
    return (localStorage.getItem('sinotruck_quotation_view_mode') as 'table' | 'card') || 'table';
  });
  ...
  const handleViewModeChange = (mode: 'table' | 'card') => {
    setViewMode(mode);
    localStorage.setItem('sinotruck_quotation_view_mode', mode);
  };
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('New Sinotruk quotation created:', payload);
      ...
      .on(
        ...
        (payload) => {
          console.log('Sinotruk quotation updated:', payload);
      ...
      .on(
        ...
        (payload) => {
          console.log('Sinotruk quotation deleted:', payload);
  ...
  const handleViewQuotation = async (versionOrId: SinotrukQuotation | { id: string }) => {
  ...
  const handleEditQuotation = async (versionOrId: SinotrukQuotation | { id: string }) => {
    const id = typeof versionOrId === 'object' && 'id' in versionOrId ? versionOrId.id : '';
    if (!id) return;
  ...
  const handleGenerateInvoice = (quotation: SinotrukQuotation) => {
    setSelectedQuotation(quotation);
    setInvoiceGeneratorOpen(true);
  };

  const handleRepeatQuotation = (quotation: SinotrukQuotation) => {
    setSelectedQuotation(quotation);
    setRepeatModalOpen(true);
  };

  const columns: ColumnDef<SinotrukQuotation>[] = [
    {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Sinotruk Bus Quotations</CardTitle>
        ...
        <CardContent>
          {viewMode === 'table' ? (
            ...
            <SinotrukCustomerCardView canManageLinks={canManageLinks} />
          )}
        </CardContent>
      </Card>

      <SinotrukQuotationViewModal
