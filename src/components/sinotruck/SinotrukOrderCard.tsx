...
import { Eye, User, Building2, Truck, Calendar, CreditCard } from 'lucide-react';
...
interface SinotrukOrderCardProps {
    order: SinotrukOrder & { sinotruck_quotations?: any };
    onViewDetails: (order: SinotrukOrder) => void;
    compact?: boolean;
}

export function SinotrukOrderCard({ order, onViewDetails, compact = false }: SinotrukOrderCardProps) {
    if (!order) return null;

    const quotation = order.sinotruck_quotations;
    const customerName = quotation?.customer_name || 'N/A';
    // Note: sinotruck_quotations might not have company_name based on previous file checks, but let's keep it safe
    ...
    if (compact) {
        return (
            <div className="bg-muted/50 rounded-lg p-3 border border-border/50">
                ...
                <div className="text-sm text-muted-foreground mb-2">{order.truck_model}</div>
            ...
            <CardContent className="p-4">
                ...
                <div className="space-y-2 mb-4 text-sm">
                    ...
                    <div className="flex items-center gap-2 text-muted-foreground">
                        ...
                        <span>{order.truck_model} × {order.quantity}</span>
                    </div>
                    <div className="flex items-center gap-2 text-muted-foreground">
