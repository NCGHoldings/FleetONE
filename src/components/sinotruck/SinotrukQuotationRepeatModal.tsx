...
interface SinotrukQuotationRepeatModalProps {
...
export function SinotrukQuotationRepeatModal({
  quotation,
  open,
  onClose,
  onSuccess,
}: SinotrukQuotationRepeatModalProps) {
    ...
    try {
      ...
      if (addOns && addOns.length > 0 && newQuotations.length > 0) {
        for (const newQuotation of newQuotations) {
          ...
          await supabase.from('sinotruck_quotation_addons').insert(addOnsCopy);
        }
      }
      ...
      if (originalSignatures && originalSignatures.length > 0 && newQuotations.length > 0) {
        for (const newQuotation of newQuotations) {
          ...
          await supabase.from('sinotruck_quotation_signatures').insert(signaturesCopy);
        }
      }
