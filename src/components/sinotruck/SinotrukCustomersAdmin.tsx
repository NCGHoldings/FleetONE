...
export default function SinotrukCustomersAdmin() {
  ...
  const loadCustomers = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          parent_customer:sinotruck_customers!parent_customer_id(company_name)
        `)
  ...
  const handleSyncToAccounting = async (customer: Customer) => {
    try {
      ...
      if (result.success) {
        ...
        loadCustomers(); // Reload to reflect any potential updates
      } else {
        toast({
          title: "Sync Failed",
