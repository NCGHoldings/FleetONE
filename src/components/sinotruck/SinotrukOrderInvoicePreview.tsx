...
interface SinotrukOrderInvoicePreviewProps {
  invoiceRecordId: string;
  invoiceData: any;
  selectedTemplate?: VehicleSalesTemplate | null;
}

export function SinotrukOrderInvoicePreview({ invoiceRecordId, invoiceData, selectedTemplate }: SinotrukOrderInvoicePreviewProps) {
  const [signatures, setSignatures] = useState<SinotrukInvoiceSignature[]>([]);
  const { fetchSignatures } = useSinotrukInvoiceSignatures();
  ...
  const templateTypeCode = isTaxInvoice ? 'sinotruck_order_invoice' : 'sinotruck_order_invoice';
  ...
  const invoiceHTML = generateSinotrukOrderInvoiceHTML(mergedInvoiceData);
