...
import { useSinotruckCashReceipts } from '@/hooks/useSinotruckCashReceipts';
import { supabase } from '@/integrations/supabase/client';
import { Pencil, Type, Upload, UserCheck } from 'lucide-react';

interface SinotruckCashReceiptSignatureModalProps {
  isOpen: boolean;
  onClose: () => void;
  receiptId: string;
  defaultRole?: 'customer' | 'finance';
  defaultSignerName?: string;
  onSignatureSaved: () => void;
}

export const SinotruckCashReceiptSignatureModal = ({
  isOpen,
  onClose,
  receiptId,
  defaultRole,
  defaultSignerName,
  onSignatureSaved
}: SinotruckCashReceiptSignatureModalProps) => {
  const [signatureRole, setSignatureRole] = useState<'customer' | 'finance'>(defaultRole || 'customer');
  ...
  const { loading, updateReceiptSignature } = useSinotruckCashReceipts();

  useEffect(() => {
    if (isOpen) {
      loadProfileSignature();
      setSignatureRole(defaultRole || 'customer');
  ...
  const handleSave = async () => {
    ...
    const success = await updateReceiptSignature(
      receiptId,
      signatureRole,
      signerName,
      signatureData,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Add Cash Receipt Signature</DialogTitle>
        </DialogHeader>
