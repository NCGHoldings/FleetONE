...
import { useSinotrukVehicleDataManagement, ColumnMapping } from '@/hooks/useSinotrukVehicleDataManagement';
...
function isSectionHeaderRow(row: any[], mappedFields: Record<number, string | null>): boolean {
  ...
  if (emptyCount === checkedFields.length) {
    ...
    return firstCell.length > 0 && isNaN(Number(firstCell));
  }
  return false;
}

export function SinotrukVehicleDataUpload({ onUploadComplete }: Props) {
  ...
  const { autoDetectColumnMapping, createDataSheet, insertVehicleRecords, isLoading } = useSinotrukVehicleDataManagement();

  const fetchShipments = async () => {
    const { data } = await supabase
      .from('sinotruck_shipment_groups')
  ...
  const handleCreateShipment = async () => {
    ...
    try {
      const { data, error } = await (supabase
        .from('sinotruck_shipment_groups') as any)
        .insert({
