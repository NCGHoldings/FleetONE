...
interface SinotrukQuotationFormProps {
  onSubmit: () => void;
  onCancel: () => void;
}
...
export function SinotrukQuotationForm({ onSubmit, onCancel }: SinotrukQuotationFormProps) {
    ...
    try {
      ...
      try {
        const syncResult = await syncToAccounting({
          ...
          source_module: 'sinotruck',
          source_record_id: insertedData.id,
        });
        if (syncResult.success) {
          console.log(`[CustomerBridge] Sinotruck customer ${syncResult.isNew ? 'created' : 'linked'} in accounting: ${syncResult.customerId}`);
        }
      } catch (syncError) {
  ...
  return (
    <Dialog open={true} onOpenChange={onCancel}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Create Sinotruk Bus Quotation</DialogTitle>
        </DialogHeader>
