...
import { EnhancedSinotrukOrderDetailsModal } from './EnhancedSinotrukOrderDetailsModal';
import { SinotrukCreateOrderModal } from './SinotrukCreateOrderModal';
import { SinotrukOrderCard } from './SinotrukOrderCard';
import { SinotrukShipmentGroupManagement } from './SinotrukShipmentGroupManagement';
...
export function SinotrukOrdersList() {
    const [orders, setOrders] = useState<SinotrukOrder[]>([]);
    const [loading, setLoading] = useState(true);
    const [selectedOrder, setSelectedOrder] = useState<SinotrukOrder | null>(null);
    const [showOrderDetails, setShowOrderDetails] = useState(false);
    const [showCreateOrder, setShowCreateOrder] = useState(false);
    const [viewMode, setViewMode] = useState<'table' | 'card'>('table');
    const [showShipments, setShowShipments] = useState(false);
    const { getOrdersWithDetails, updateOrderPhase } = useSinotrukOrderManagement();
    ...
    const loadOrders = async () => {
        try {
            ...
            if (result.success) {
                setOrders((result.orders || []) as SinotrukOrder[]);
            }
        } catch (error) {
    ...
    const getPhaseBadge = (phase: string) => {
        ...
        const colorClass = phaseColors[phase] || 'bg-gray-100 text-gray-800';
    ...
    const columns: ColumnDef<SinotrukOrder>[] = [
        {
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div>
                    <div className="font-medium">{row.getValue("truck_model")}</div>
        ...
        },
        {
            header: "Customer",
            cell: ({ row }) => {
                const quotation = (row.original as any).sinotruck_quotations;
    ...
    if (showShipments) {
        return (
            <div className="space-y-4">
                ...
                <SinotrukShipmentGroupManagement />
            </div>
        );
    }
    ...
    return (
        <>
            ...
            {showOrderDetails && selectedOrder && (
                <EnhancedSinotrukOrderDetailsModal
                    order={selectedOrder}
