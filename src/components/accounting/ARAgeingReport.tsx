...
import { useARInvoices, useCustomers } from "@/hooks/useAccountingData";
...
export const ARAgeingReport = () => {
  const { data: invoices, isLoading } = useARInvoices();
  const { data: customers } = useCustomers();

  const ageingData = useMemo(() => {
    if (!invoices) return { customerAgeing: [], totals: null };
      ...
      .forEach(inv => {
        ...
        if (!customerMap.has(customerId)) {
          ...
          customerMap.set(customerId, {
            customerId,
            customerName: customer?.customer_name || inv.customers?.customer_name || "Unknown",
            customerCode: customer?.customer_code || inv.customers?.customer_code || "",
    ...
    const customerAgeing = Array.from(customerMap.values()).sort((a, b) => 
      b.buckets.total - a.buckets.total
    );

    const totals: AgeingBucket = customerAgeing.reduce(
      (acc, curr) => ({
    ...
    return { customerAgeing, totals };
  }, [invoices, customers]);

  const { customerAgeing, totals } = ageingData;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">AR Ageing Report</h2>
          <p className="text-sm text-muted-foreground">
            Accounts receivable by ageing bucket as of {new Date().toLocaleDateString()}
          </p>
        </div>
              ...
              {customerAgeing.map((customer) => (
                <tr key={customer.customerId} className="border-b hover:bg-muted/50">
                  ...
                  <td className="text-right py-3 px-2 text-green-600">
                    <CurrencyDisplay amount={customer.buckets.current} />
                  </td>
                  <td className="text-right py-3 px-2 text-yellow-600">
                    <CurrencyDisplay amount={customer.buckets.days1to30} />
                  </td>
                  <td className="text-right py-3 px-2 text-orange-600">
                    <CurrencyDisplay amount={customer.buckets.days31to60} />
                  </td>
                  <td className="text-right py-3 px-2 text-red-500">
                    <CurrencyDisplay amount={customer.buckets.days61to90} />
                  </td>
                  <td className="text-right py-3 px-2 text-destructive">
                    <CurrencyDisplay amount={customer.buckets.over90} />
                  </td>
                  <td className="text-right py-3 px-2 font-semibold">
                    <CurrencyDisplay amount={customer.buckets.total} />
                  </td>
                </tr>
              ))}
            </tbody>
