...
-- Grant accounting access to all existing super_admin, admin, and finance users (using DISTINCT to avoid duplicates)
INSERT INTO public.user_page_permissions (user_id, page_identifier, has_access, granted_by)
SELECT DISTINCT
  ur.user_id,
  'accounting',
...
WHERE ur.role IN ('super_admin', 'admin', 'finance')
ON CONFLICT (user_id, page_identifier) 
DO UPDATE SET 
  has_access = true, 
  granted_at = now();