...
BEGIN
  ...
  -- The edge function and frontend use 'SBO' for School Bus business unit, but source_module might not always be set
  -- So we enforce it if source_module = 'school_bus' OR business_unit_code = 'SBO'
  -- Wait, the original trigger only checked v_module = 'school_bus'. Let's stick to the original condition 
  -- but also check business_unit_code for robust enforcement if source_module is missing.
  ...
  IF v_module = 'school_bus' OR EXISTS (SELECT 1 FROM journal_entries WHERE id = NEW.journal_entry_id AND business_unit_code = 'SBO') THEN
    ...
    -- 2. Fetch the global suspense account
