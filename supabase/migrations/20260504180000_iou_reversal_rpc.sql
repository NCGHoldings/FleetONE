-- Migration: 20260504180000_iou_reversal_rpc.sql
-- Description: RPCs for safely deleting and reversing IOUs
...
BEGIN
    ...
    -- 2. Find related Journal Entries by reference and get GL impacts
    SELECT 
        COUNT(DISTINCT je.id), 
        ARRAY_AGG(DISTINCT je.entry_number),
        COALESCE(
            jsonb_agg(
                jsonb_build_object(
    ...
    INTO v_je_count, v_je_numbers, v_je_impacts
    ...
    WHERE je.reference = v_iou_number OR je.reference = 'IOU-' || substr(p_iou_id::text, 1, 8);

    -- 3. Find related Petty Cash Transactions
    SELECT 
        COUNT(DISTINCT pct.id), 
    ...
    v_result = jsonb_build_object(
        ...
        'je_impacts', COALESCE(v_je_impacts, '[]'::jsonb),
...
BEGIN
    ...
    -- 1.5 Break Foreign Keys to allow deletion of related records
    UPDATE public.iou_records SET journal_entry_id = NULL WHERE id = p_iou_id;
