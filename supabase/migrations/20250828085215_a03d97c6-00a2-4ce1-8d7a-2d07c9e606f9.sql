-- Check current structure and create missing tables for Special Hire system
...
-- Insert default data
INSERT INTO public.bus_types (name, capacity, avg_km_per_l, features) VALUES
('Standard Bus', 45, 8.0, 'Standard seating, Air conditioning'),
('Luxury Bus', 35, 7.0, 'Luxury seating, Air conditioning, Entertainment system'),
('Mini Bus', 25, 10.0, 'Compact size, Good for small groups')
ON CONFLICT DO NOTHING;

INSERT INTO public.fuel_settings (diesel_price_lkr_per_l, parking_location_name) VALUES
(350.0, 'Main Depot')
ON CONFLICT DO NOTHING;

-- Insert sample rate cards
INSERT INTO public.hire_rate_cards (hire_type, bus_type_id, from_km, to_km, rate_per_km_lkr) 
SELECT 
  'Outside', 
  bt.id, 
  0, 
  50, 
  CASE 
    WHEN bt.name = 'Standard Bus' THEN 45.0
    WHEN bt.name = 'Luxury Bus' THEN 65.0
    WHEN bt.name = 'Mini Bus' THEN 35.0
  END
FROM public.bus_types bt
ON CONFLICT DO NOTHING;