-- Fix: Update get_public_bus_assignment to also check fleet_master_roster
-- The fleet_master_roster is the authoritative source for bus-to-route mapping
-- Previously, the RPC only checked daily_trips and buses.route (which was NULL for most buses)
...
BEGIN
    ...
    IF v_bus_id IS NULL THEN
        ...
        RETURN QUERY SELECT ''::TEXT, ''::TEXT, ''::TEXT, 0::BIGINT;
        RETURN;
    END IF;
